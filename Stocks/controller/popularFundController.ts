...
import topStock from '../models/popularfunds';
import { IPopularFund } from '../types/PopularFund'; // Adjust the import path as needed

// Add single or multiple top stocks
export const addTopStocks = async (req: Request, res: Response): Promise<void> => {
  try {
    const data: IPopularFund[] = Array.isArray(req.body) ? req.body : [req.body];
