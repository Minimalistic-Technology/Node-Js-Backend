...
import { ITopIndexFuture } from '../types/TopIndexFuture'; // Adjust the import path as necessary

// Add single or multiple top stocks
export const addTopStocks = async (req: Request, res: Response): Promise<void> => {
  try {
    const data: ITopIndexFuture[] = Array.isArray(req.body) ? req.body : [req.body];
