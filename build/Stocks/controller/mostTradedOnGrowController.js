...
const mostTradedOnGrowModel_1 = require("../../models/stocks/mostTradedOnGrowModel");
// Add one or multiple stocks
const addMostTradedOnGrow = async (req, res) => {
    try {
        const data = Array.isArray(req.body) ? req.body : [req.body];
        const result = await mostTradedOnGrowModel_1.MostTradedOnGrow.insertMany(data);
...
const getAllMostTradedOnGrow = async (_req, res) => {
    try {
        const data = await mostTradedOnGrowModel_1.MostTradedOnGrow.find({}, 'name price change image');
...
const getMostTradedOnGrowById = async (req, res) => {
    try {
        const stock = await mostTradedOnGrowModel_1.MostTradedOnGrow.findById(req.params.id, '-name -price -change -image -__v');
        if (!stock) {
...
const updateMostTradedOnGrow = async (req, res) => {
    try {
        const updated = await mostTradedOnGrowModel_1.MostTradedOnGrow.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
        });
        if (!updated) {
...
const deleteMostTradedOnGrow = async (req, res) => {
    try {
        const deleted = await mostTradedOnGrowModel_1.MostTradedOnGrow.findByIdAndDelete(req.params.id);
        if (!deleted) {
            res.status(404).json({ error: 'Stock not found' });
            return;
        }
