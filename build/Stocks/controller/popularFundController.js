...
const popularfunds_1 = __importDefault(require("../models/popularfunds"));
// Add single or multiple top stocks
const addTopStocks = async (req, res) => {
    try {
        const data = Array.isArray(req.body) ? req.body : [req.body];
        const result = await popularfunds_1.default.insertMany(data);
...
const getTopStocks = async (_req, res) => {
    try {
        const data = await popularfunds_1.default.find({}, 'name price change image');
...
const getTopStockById = async (req, res) => {
    try {
        const stock = await popularfunds_1.default.findById(req.params.id, '-name -price -change -image -__v');
        if (!stock) {
...
const updateTopStock = async (req, res) => {
    try {
        const updated = await popularfunds_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updated) {
            res.status(404).json({ error: 'Stock not found' });
            return;
        }
...
const deleteTopStock = async (req, res) => {
    try {
        const deleted = await popularfunds_1.default.findByIdAndDelete(req.params.id);
        if (!deleted) {
            res.status(404).json({ error: 'Stock not found' });
            return;
        }
