...
const mostTradedOnGrowModel_1 = __importDefault(require("../models/mostTradedOnGrowModel"));
...
const getMostTradedOnGrowById = async (req, res) => {
    try {
        const stock = await mostTradedOnGrowModel_1.default.findById(req.params.id, '-name -price -change -image');
        if (!stock) {
