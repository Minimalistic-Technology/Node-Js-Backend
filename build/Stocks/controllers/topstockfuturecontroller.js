...
const addTopStocks = async (req, res) => {
    try {
        ...
        const result = await TopStockFuture_1.default.insertMany(data);
...
const getTopStocks = async (_req, res) => {
    try {
        const data = await TopStockFuture_1.default.find({}, 'name price change image');
...
const getTopStockById = async (req, res) => {
    try {
        const stock = await TopStockFuture_1.default.findById(req.params.id, '-name -price -change -image -__v');
        if (!stock) {
...
const updateTopStock = async (req, res) => {
    try {
        const updated = await TopStockFuture_1.default.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
        });
        if (!updated) {
...
const deleteTopStock = async (req, res) => {
    try {
        const deleted = await TopStockFuture_1.default.findByIdAndDelete(req.params.id);
        if (!deleted) {
            res.status(404).json({ error: 'Stock not found' });
            return;
        }
