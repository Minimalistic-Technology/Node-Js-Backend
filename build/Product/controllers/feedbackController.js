...
const postFeedback = async (req, res) => {
    try {
        const feedback = new feedback_1.FeedbackModel(req.body);
...
const updateFeedback = async (req, res) => {
    try {
        const updated = await feedback_1.FeedbackModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updated) {
...
const deleteFeedback = async (req, res) => {
    try {
        const deleted = await feedback_1.FeedbackModel.findByIdAndDelete(req.params.id);
        if (!deleted) {
            res.status(404).json({ error: 'Feedback not found' });
            return;
        }
