...
export const getTestsByLetter = async (req: Request, res: Response) => {
  try {
...
export const addMultipleTests = async (req: Request, res: Response) => {
  try {
...
export const appendToTestById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
export const appendTestsByLetter = async (req: Request, res: Response) => {
  try {
    const { letter } = req.params;
    if (!letter || letter.length !== 1 || !/[A-Z]/i.test(letter)) {
      return res.status(400).json({ message: 'Invalid letter' });
