...
import topStockModel from '../models/popularfunds';

// Define the interface for the top stock document
...
export const addTopStocks = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await topStockModel.insertMany(data);
...
export const getTopStocks = async (_req: Request, res: Response): Promise<void> => {
  try {
    const data = await topStockModel.find({}, 'name price change image');
...
export const getTopStockById = async (req: Request, res: Response): Promise<void> => {
  try {
    const stock = await topStockModel.findById(req.params.id, '-name -price -change -image -__v');
    if (!stock) {
...
export const updateTopStock = async (req: Request, res: Response): Promise<void> => {
  try {
    const updated = await topStockModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!updated) {
      res.status(404).json({ error: 'Stock not found' });
      return;
    }
...
export const deleteTopStock = async (req: Request, res: Response): Promise<void> => {
  try {
    const deleted = await topStockModel.findByIdAndDelete(req.params.id);
    if (!deleted) {
      res.status(404).json({ error: 'Stock not found' });
      return;
    }
