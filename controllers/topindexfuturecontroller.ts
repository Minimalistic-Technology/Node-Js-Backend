...
import topStock from '../models/TopIndexFuture';

// Define interface for a single Top Index Future document
interface TopIndexFuture extends Document {
  _id: Types.ObjectId;
  name: string;
  price: number;
  change: number;
...
export const addTopStocks = async (req: Request, res: Response): Promise<void> => {
  try {
    const data: Partial<TopIndexFuture>[] = Array.isArray(req.body) ? req.body : [req.body];
